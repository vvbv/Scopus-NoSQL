...
mongoimport --db proyecto_nosql --collection articles --file /dataset/mongo_dataset.json --port 27017 > /dev/null
echo "[$(date +"%T")][Mongo]: Documentos importados."
export DBA_PASSWORD=root 
echo "[$(date +"%T")][Virtuoso]: Configurando servidor."
/bin/bash /entrypoint/virtuoso_entrypoint.sh > /dev/null
